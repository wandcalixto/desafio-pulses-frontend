<template>

  <div id="app">
    <div class="container">
        <br>
        <h3>Apresentação</h3>
        <br>
        <b>Olá,</b>
        <br>
        Eu me chamo Wanderson Calixto da Silva, sou formado em Ciência da Computação e trabalho há três anos como programador.<br>
        Esta aplicação foi criada a partir do Desafio Prático do processo seletivo da Agência Zetta.<br>
        Entre as diversas tecnologias utilizadas no desenvolvimento, pode-se listar:
       <ul>
            <li><b>Backend</b><ul>
                    <li>Linguagem: Java</li>
                    <li>Framework Java: Spring Boot</li>
                    <li>Banco de Dados: PostgreSQL</li>
                    <li>Framework para persistencia dos dados: Hibernate</li>
                </ul>
            </li>
        </ul>
        <ul>
            <li><b>Frontend</b><ul>
                    <li>Framework Javascript: Vue.js</li>
                    <li>Framework Web: BootStrap</li>                  
                </ul>
            </li>
        </ul>
         <ul>
            <li><b>Hospedagem</b><ul>
                    <li>Backend: <a href="https://apirest-zetta.herokuapp.com/swagger-ui.html#/" target="_blank">Heroku</a></li>
                    <li>Frontend: <a href="http://wandersonzetta.online/#/" target="_blank">Umbler</a></li>                  
                </ul>
            </li>
        </ul>
        <ul>
            <li><b>Repostório</b><ul>
                    <li><a href="https://github.com/wandcalixto/zetta-backend" target="_blank">Backend</a></li>
                    <li><a href="https://github.com/wandcalixto/zetta-frontend" target="_blank">Frontend</a></li> 
                    <li><a href="https://drive.google.com/file/d/1J9qae63PRTfX7swFnwie_FwOhyQcpzpD/view?usp=sharing" target="_blank">Instruções</a></li>      
                               
                </ul>
            </li>
        </ul>
    </div>
  </div>
</template>

<script>
export default{
  name: "apresentacao"
}
</script>

<style>
 

</style>
